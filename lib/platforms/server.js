...
// todo: replace default path with cdn url
const resolveEnginePath = (path = '/node_modules/swayer/index.js') => path;
...
class HtmlRenderer extends createServerRenderer(ElementRenderer) {
  ...
  setSystemAttrs(input) {
    ...
    const data = {
      ...
      [MODE_ATTR]: this.engine.mode,
      [ROUTE_ATTR]: this.engine.routingPath,
    };
...
export default class ServerPlatform {
  ...
  createSPA(title = 'Index', lang = 'en') {
    ...
    const srcPath = resolveSourcePath(ENTRY_FILE_NAME);
    return `${this.#doctype}
<html lang="${lang}"
...
>
  ...
  #createEngine(entryPath, routingPath) {
    ...
    const mode = this.#options.mode || CSR_MODE;
