...
class BrowserPlatform {
  ...
  #createEngine(mode, route, styles) {
    const routingPath = this.#location.pathname;
    if (routingPath !== route) throw Reporter.error('SchemaMismatch');
    const baseURI = this.#dom.baseURI;
    const Engine = mode === CSR_MODE ? CSREngine : SwayerEngine;
    this.#engine = new Engine(this.#webApi, baseURI, routingPath);
  ...
  #createSPAEngine() {
    const routingPath = this.#location.pathname;
    const baseURI = this.#dom.baseURI;
    this.#engine = new SwayerEngine(this.#webApi, baseURI, routingPath);
    this.#engine.setRendererTypes({
