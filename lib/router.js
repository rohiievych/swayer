...
import { is, isBrowser, normalizePath } from './utils.js';
...
const createRoutingPath = (webApi) => {
  const { origin, href } = webApi.location;
  const baseURI = webApi.document.baseURI;
...
// todo fix relative navigation (styles are not rendered),
// e.g. this.router.go('todos/1')
...
export default class Router {
  ...
  static registerHistoryChanges(router) {
    ...
    engine.webApi.addEventListener('popstate', () => {
      const path = createRoutingPath(engine.webApi);
  ...
  finishRouting() {
    ...
    if (isNotFound) {
      ...
      const anyRoute = hasSegments
        ...
        : null;
      if (anyRoute) this.reload(anyRoute.schema);
  ...
  async #findRoute() {
    ...
    for (const route of this.#routes) {
      ...
      // todo investigate
      // if (route.pattern === EMPTY_PATTERN && pathLen === 0) {
      //   match = route;
      //   break;
      // }
  ...
  #pushState(path, state = {}) {
    if (isBrowser) {
      ...
      if (path.startsWith(PATH_DELIMITER)) {
        const base = this.engine.options.basePath;
        nextPath = path.replace(PATH_DELIMITER, base);
      }
      nextPath = normalizePath(nextPath);
      this.engine.webApi.history.pushState(state, '', nextPath);
    }
  }
