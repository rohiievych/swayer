...
export class ElementRenderer extends Renderer {
  ...
  render() {
    this.#setStaticStyles(this.context);
    return super.render();
  }

  replace(context) {
    this.#setStaticStyles(context);
  ...
  #setStaticStyles(context) {
    const { schema: { styles }, binding } = context;
    const newClass = this.#staticStyler.setStyles(binding, styles);
    context.setSystemClass('static', newClass);
  }
}
...
export class HeadRenderer extends ElementRenderer {
  ...
  render() {
    ...
    head.append(base, ...this.#stylesMap.values());
    super.render();
  ...
  replace(context) {
    ...
    const base = this.#createBase();
    newHead.append(base, ...this.#stylesMap.values());
