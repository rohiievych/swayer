...
export class NodeBinding {
  ...
  setAttribute(attrName, value) {
    ...
    const attr = camelToKebabCase(attrName);
  ...
  setClasses(classes, systemClasses) {
    let className;
    if (is.str(classes)) className = classes;
    else if (is.arr(classes)) className = classes.join(' ');
    else return;
    if (systemClasses) className = `${systemClasses} ${className}`;
...
export class ElementBinding extends NodeBinding {
  constructor(webApi, schema, existing) {
    ...
    const { text, classes, attrs } = schema;
    if (!is.fn(text)) this.setText(text);
    if (!is.fn(classes)) this.setClasses(classes);
    if (!is.fn(attrs)) this.setAttributes(attrs);
  }
}
