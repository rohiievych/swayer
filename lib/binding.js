  ...
  setClasses(value) {
    ...
    if (classList.length > 0) {
      for (const clas of classList) {
        if (clas.startsWith(CLASS_PREFIX)) systemClasses.push(clas);
      }
    }
    const classes = is.str(value) ? value.split(' ') : value;
    this.#node.className = [...systemClasses, ...classes].join(' ');
  }
}
...
export class ElementBinding extends NodeBinding {
  constructor(webApi, schema, existing) {
    ...
    const { text, classes, props, attrs } = schema;
    if (!is.fn(text)) this.setText(text);
    if (!is.fn(classes)) this.setClasses(classes);
    if (!is.fn(props)) this.setProperties(props);
