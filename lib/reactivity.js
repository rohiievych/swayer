...
class DependencyHandler {
  ...
  static #compare(newValue, oldValue) {
    const first = is.obj(newValue) ? newValue[TARGET] : newValue;
    const second = is.obj(oldValue) ? oldValue[TARGET] : oldValue;
...
export default class Reactivity {
  ...
  register(updateTarget, prop, source) {
    ...
    if (!this.#component.model) throw Reporter.error('ModelNotFound', source);
