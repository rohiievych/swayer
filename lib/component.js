...
export class ReactivityManager {
  ...
  registerReactivity() {
    ...
    const setClasses = (val) => binding.setClasses(val);
  ...
  #registerStyles() {
    ...
    const changeStyles = (value) => {
      ...
      classes.static = styler.changeStyles(binding, prevClass, value);
    };
    const compStyler = stylers[COMPUTED_STYLE_TYPE];
    const changeCompStyles = (value) => {
      const prevClass = classes.computed;
      classes.computed = compStyler.changeStyles(binding, prevClass, value);
    };
    const changeCompStyleProp = (index, value) => {
      const prevClass = classes.computedArray[index];
      classes.computedArray[index] = compStyler.changeStyles(
        binding, prevClass, value,
      );
    };
    const stylesSrc = schema.styles;
    this.#registerProp(stylesSrc, changeStyles);
    this.#registerProp(stylesSrc?.compute, changeCompStyles);
    this.#registerObj(stylesSrc?.compute, changeCompStyleProp);
  }
