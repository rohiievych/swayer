...
export class Context extends BaseContext {
  ...
  constructor(schema, originalSchema, parent, module) {
    ...
    this.stylers = this.parent?.stylers;
  ...
  createStyler(styleType, ruleHandler, classes) {
    const styler = new Styler(ruleHandler, classes);
    if (is.nullish(this.stylers)) this.stylers = {};
    return (this.stylers[styleType] = styler);
  }
  ...
  getSystemClassNames() {
    const { static: stat, computed, computedArray } = this.styleClasses;
    const classes = `${stat} ${computed} ${computedArray.join(' ')}`;
    return classes.trim();
  }

  setSystemClass(type, className, index) {
    if (is.nullish(index)) this.styleClasses[type] = className;
    else this.styleClasses[type][index] = className;
  }
  ...
  #callHook(hook) {
    const hookHandler = this.schema.hooks?.[hook];
    if (hookHandler) return hookHandler.call(this.component);
  }
...
export class ContextFactory {
  ...
  async createContext(inputSchema, parent, tail) {
    if (!inputSchema) return false;
    if (!tail) tail = { module: parent?.module, recurrent: [] };
    let schema = inputSchema;
    if (is.basic(schema)) {
      const text = schema.toString();
      return new BaseContext(text, schema, parent, tail.module);
    }
    ...
    if (schema.routes) {
      ...
      if (match) return this.createContext(match, parent, tail);
      return false;
    }
    if (schema.path) {
      const loader = this.#engine.loader;
      tail.module = await loader.loadSchemaModule(schema);
      schema = tail.module.schema;
      if (!parent && is.arr(schema)) schema = { children: schema };
    }
    if (is.fn(schema)) {
      const reactiveInput = parent.reactivityManager.registerChildren(schema);
      return this.createContext(reactiveInput, parent, tail);
    }
    if (is.arr(schema)) {
      const recur = (nested) => this.createContext(nested, parent, tail);
      const recurred = await Promise.all(schema.map(recur));
      const contexts = recurred.filter(
        (schema) => schema && !is.arr(schema),
      );
      tail.recurrent.push(...contexts);
      return tail.recurrent;
    }
    schema = this.#validateSchema(schema, tail.module.url);
    this.#initNamespaces(schema);
    this.#preloadModules(schema);
    return new Context(schema, originalSchema, parent, tail.module);
  }
  ...
  #setupRouter(schema, parent) {
    ...
    if (!router.parent) {
      const initialRoutingPath = this.#engine.options.routingPath;
      router.setInitialPath(initialRoutingPath);
...
export default class SwayerEngine {
  ...
  async *start(startContext) {
    ...
    const collectFinalizers = (context) => {
      const { schema, router } = context;
      if (schema.hooks?.ready) readiness.push(context);
